...
export default function JuguangPage() {
  ...
  function selectPreset(value: DatePreset) {
    const nextRange = value === 'custom' ? { startDate, endDate } : presetRange(value);
  ...
  useEffect(() => {
    ...
    const reconcileRange = async () => {
      ...
      try {
        ...
        if (attempts >= 60) {
          setRangeSyncing(false);
          toast.warning(result.hasCachedSnapshot
            ? '后台更新耗时较长，当前继续展示最近一次完整数据'
            : '当前时间范围首次同步时间较长，可稍后点击手动刷新');
          return;
        }
