...
CREATE TABLE `salary_records` (
  ...
  `experience_fee` decimal(10,2) DEFAULT 0 COMMENT '体验卡手工费',
  `labor_fee` decimal(10,2) DEFAULT 0 COMMENT '手工费',
  `commission` decimal(10,2) DEFAULT 0 COMMENT '提成',
  `coupon_fee` decimal(10,2) DEFAULT 0 COMMENT '抵扣券',
  `other_fee` decimal(10,2) DEFAULT 0 COMMENT '其他费用',
  `deduction` decimal(10,2) DEFAULT 0 COMMENT '扣减费用',
  `total` decimal(10,2) DEFAULT 0 COMMENT '总额',
  `status` enum('待结算','审核中','已结算') DEFAULT '待结算' COMMENT '状态',
  `confirmed_at` datetime DEFAULT NULL COMMENT '确认时间',
  `confirmed_by` varchar(36) DEFAULT NULL COMMENT '确认人',
  `settled_at` datetime DEFAULT NULL COMMENT '结算时间',
  `settlement_note` varchar(500) DEFAULT NULL COMMENT '结算备注',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  PRIMARY KEY (`id`),
...
-- 工资结算凭证明细（由已完成排期服务幂等同步）
CREATE TABLE `salary_settlement_entries` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='工资结算凭证明细';

-- 技师客户月度结算调整（抵扣券、其他费用及历史纠偏）
CREATE TABLE `salary_customer_adjustments` (
  `id` varchar(36) NOT NULL,
  `therapist_id` varchar(36) NOT NULL,
  `customer_id` varchar(36) NOT NULL,
  `month` varchar(7) NOT NULL,
  `coupon_fee` decimal(10,2) NOT NULL DEFAULT 300,
  `other_fee` decimal(10,2) NOT NULL DEFAULT 0,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='技师客户月度结算调整';
